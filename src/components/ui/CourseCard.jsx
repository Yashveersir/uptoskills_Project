...
const FALLBACK_IMG = "https://images.unsplash.com/photo-1501504905252-473c47e087f8?q=80&w=800&auto=format&fit=crop";

const CourseCard = ({ course }) => {
  if (!course) return null;

  const teacherInitial = (course.teacher || "M")[0].toUpperCase();
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center border border-primary-400 overflow-hidden shadow-sm shrink-0">
            {course.teacherImage ? (
              <img
                src={course.teacherImage}
                alt={course.teacher || "Mentor"}
                className="w-full h-full object-cover"
                onError={(e) => { e.currentTarget.style.display = "none"; }}
              />
            ) : (
              <span className="text-white text-xs font-bold">{teacherInitial}</span>
            )}
          </div>
          <p className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 truncate">
            {course.teacher || "Expert Mentor"}
          </p>
        </div>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-1.5 text-neutral-500 dark:text-neutral-400">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider">{course.level || "All Levels"}</span>
          </div>
          <div className="flex items-center gap-1.5 text-neutral-500 dark:text-neutral-400">
            <Users size={14} className="text-primary-500" />
            <span className="text-[10px] font-bold uppercase tracking-wider">{course.students || "0"}</span>
          </div>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-5 border-t border-neutral-200 dark:border-neutral-800">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold text-neutral-400 tracking-tighter">({course.reviews || "0"})</span>
          </div>
