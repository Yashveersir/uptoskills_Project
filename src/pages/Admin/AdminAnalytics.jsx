...
// Icon map so demo stats always have their icon
const STAT_ICONS = {
  "Total Revenue": <DollarSign size={20} />,
  "Active Students": <Users size={20} />,
  "Course Completion": <TrendingUp size={20} />,
  "Platform Rating": <BarChart3 size={20} />,
};

const DEMO_ANALYTICS = {
  stats: [
    { label: "Total Revenue", value: "$45,231", trend: "+12.5%", isUp: true },
    { label: "Active Students", value: "1,024", trend: "+18.2%", isUp: true },
...
const AdminAnalytics = () => {
  ...
  const stats = (analytics?.stats ?? DEMO_ANALYTICS.stats).map((s) => ({
    ...s,
    icon: STAT_ICONS[s.label] ?? <BarChart3 size={20} />,
  }));

  const enrollmentTrend = analytics?.enrollmentTrend ?? DEMO_ANALYTICS.enrollmentTrend;
  const categories = analytics?.categories ?? DEMO_ANALYTICS.categories;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {loading
          ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-white dark:bg-neutral-900 p-6 rounded-lg border border-neutral-200 dark:border-neutral-800 animate-pulse">
                <div className="h-12 w-12 rounded-2xl bg-neutral-200 dark:bg-neutral-800 mb-6" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-neutral-900 rounded-lg p-8 md:p-10 border border-neutral-200 dark:border-neutral-800 shadow-card">
          <div className="flex justify-between items-center mb-10">
            ...
            <select className="bg-neutral-50 dark:bg-neutral-800 border-none rounded-xl px-4 py-2 text-xs font-bold text-neutral-500 outline-none cursor-pointer">
          ...
          <div className="h-64 flex items-end justify-between gap-2 px-2">
            {enrollmentTrend.map((h, i) => (
              <div key={i} className="flex-1 group relative">
                ...
                />
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-neutral-900 text-white text-[8px] rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                  {h * 12} Learners
                </div>
              </div>
            ))}
